...
export default function ImportPanel({
    ...
    return (
      <div className="mb-4">
        <button
          ...
          className="flex w-full items-center gap-3 rounded-lg border border-dashed border-ink-300 bg-ink-50/60 p-4 text-left transition-colors hover:border-ink-500 hover:bg-ink-50"
        >
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-ink-900 text-paper">
            <svg
        ...
        {result && (
          <p role="status" className="mt-3 text-sm font-medium text-positive-700">
            {result}
          </p>
        )}
      </div>
  ...
  return (
    <div className="mb-6 rounded-lg border border-ink-200 bg-paper p-4 shadow-card sm:p-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          className="shrink-0 rounded-lg px-2 py-1 text-sm font-medium text-ink-500 hover:bg-ink-100 hover:text-ink-700"
        >
          Close
        </button>
      </div>
      ...
      {error && (
        <p
          role="alert"
          className="mt-4 rounded-lg border border-negative-100 bg-negative-50 p-3 text-sm text-negative-700"
        >
          {error}
        </p>
      )}
      ...
      {stage === "input" ? (
        <>
          <div
            className="mt-4 flex gap-1 rounded-lg bg-ink-100 p-1"
            role="tablist"
          ...
          {mode === "alerts" ? (
            <>
              ...
              <textarea
                ...
                rows={9}
                placeholder={EXAMPLE}
                className="mt-3 block w-full rounded-lg border border-ink-300 p-3 font-mono text-sm leading-6 placeholder:text-ink-400 focus:border-ink-700 focus:outline-none focus:ring-1 focus:ring-ink-700"
              />
              <div className="mt-3 flex flex-wrap items-center gap-3">
                <button
                  onClick={parseAlerts}
                  disabled={busy || !text.trim()}
                  className="inline-flex min-h-[44px] items-center gap-2 rounded-lg bg-ink-900 px-5 text-sm font-semibold text-paper transition-colors hover:bg-ink-900 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {busy ? (
                    <>
                      <Spinner />
            ...
            <>
              ...
              >
                ...
                <button
                  ...
                  disabled={busy}
                  className="mt-4 inline-flex min-h-[44px] items-center gap-2 rounded-lg bg-ink-900 px-5 text-sm font-semibold text-paper transition-colors hover:bg-ink-900 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {busy ? (
                    <>
                      <Spinner />
        ...
        <>
          ...
          {issues.length > 0 && (
            <details className="mt-3 text-sm">
              ...
              <ul className="mt-2 space-y-1 pl-4 text-ink-500">
